...
build_python:
	...
	cd build
	rm -rf *
	cmake -DCMAKE_FIND_ROOT_PATH=${CONDA_PREFIX} -DCMAKE_BUILD_TYPE=RELEASE -DCMAKE_INSTALL_PREFIX=${INSTALL_PREFIX} -DBUILD_SHARED_LIBS=${BUILD_SHARED_LIBS} -DOPENCV_EXTRA_MODULES_PATH=$(TMP_DIR)opencv/opencv_contrib-$(OPENCV_VERSION)/modules -DWITH_TIFF=OFF -DBUILD_DOCS=OFF -DBUILD_EXAMPLES=OFF -DBUILD_TESTS=OFF -DBUILD_PERF_TESTS=OFF -DBUILD_opencv_java=OFF -DBUILD_opencv_python2=OFF -DBUILD_opencv_python3=ON -DPYTHON3_LIBRARY=${CONDA_PREFIX}/lib/libpython${PYTHON_VERSION}.so -DPYTHON3_INCLUDE_DIR=${CONDA_PREFIX}/include/python${PYTHON_VERSION} -DWITH_JASPER=OFF -DOPENCV_GENERATE_PKGCONFIG=ON -DWITH_TBB=ON ..
	$(MAKE) -j $(shell nproc --all)
	$(MAKE) preinstall
	cd -
