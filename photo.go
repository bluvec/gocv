...
func FastNlMeansDenoisingColoredMulti(src []Mat, dst Mat, imgToDenoiseIndex int, temporalWindowSize int) {
	...
	C.FastNlMeansDenoisingColoredMulti(matsVector, dst.Ptr(), C.int(imgToDenoiseIndex), C.int(temporalWindowSize))
}
...
func FastNlMeansDenoisingColoredMultiWithParams(src []Mat, dst Mat, imgToDenoiseIndex int, temporalWindowSize int, h float32, hColor float32, templateWindowSize int, searchWindowSize int) {
	...
	C.FastNlMeansDenoisingColoredMultiWithParams(matsVector, dst.Ptr(), C.int(imgToDenoiseIndex), C.int(temporalWindowSize), C.float(h), C.float(hColor), C.int(templateWindowSize), C.int(searchWindowSize))
}
...
func (b *MergeMertens) Process(src []Mat, dst Mat) {
	...
	C.MergeMertens_Process((C.MergeMertens)(b.p), matsVector, dst.Ptr())
...
func (b *AlignMTB) Process(src []Mat) (dst []Mat) {
	...
	fmt.Println("intput", cSrcArray)
	C.AlignMTB_Process((C.AlignMTB)(b.p), cSrcMats, &cDstMats)
	fmt.Println("out", cDstMats.length)
	...
	for i := C.int(0); i < cDstMats.length; i++ {
		...
		dst = append(dst, NewMatByAcquirePtr(C.Mats_get(cDstMats, i)))
	}
	fmt.Println("exit", dst)
	return
}
